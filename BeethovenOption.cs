...
public class BeethovenOption
{
	...
	public string getCurrent()
	{
		...
		case BeethovenOptionType.TOGGLE:
			if ((bool)BeethovenMain.Instance.getVariableByName(this.variable))
			{
				return ": <color=#4444FFFF>ENABLED</color>";
			}
			return ": <color=#FF4444FF>DISABLED</color>";
		case BeethovenOptionType.FLOAT:
			return ": <color=#FFFF55FF>" + (float)Mathf.RoundToInt((float)BeethovenMain.Instance.getVariableByName(this.variable) * 10f) * 0.1f + "</color>";
		case BeethovenOptionType.INTEGER:
			return ": <color=#FFFF55FF>" + (int)BeethovenMain.Instance.getVariableByName(this.variable) + "</color>";
		default:
			return "";
		}
	}
	...
	public object getNextCycle()
	{
		if (this.optionType == BeethovenOptionType.INTEGER)
		{
			...
			{
				if ((int)this.cycles[i] == (int)BeethovenMain.Instance.getVariableByName(this.variable))
				{
		...
		else
		{
			...
			{
				if ((float)this.cycles[j] == (float)BeethovenMain.Instance.getVariableByName(this.variable))
				{
